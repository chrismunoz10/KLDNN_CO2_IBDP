...
p_1 = np.reshape(p_1, (Nens, Nxy, nt))
p_1 = np.transpose(p_1, (0, 2, 1)) #(Nens, nt, Nxy)

p_1_train = np.delete(p_1, test_case - 1, axis = 0).copy() #(Ntrain, Nxyz)
p_1_test = p_1[test_case - 1].copy() #(Ntrain, Nxyz)
p_1_mean = np.mean(p_1_train, axis = 0) #(nt, Nzxy)
...
"""
#Outputs
eta_component_train = np.load(f'Pressure/gamma_eta_Neta{Neta}_Ngamma{Ngamma}_NR{NR}.npy') #(Neta, train_size, Ngamma)
eta_component_test = np.load(f'Pressure/gamma_eta_test_Neta{Neta}_Ngamma{Ngamma}_NR{NR}.npy') #Neta, test_size, Ngamma)
psi_component = np.load(f'Pressure/psi_eta_Neta{Neta}_Ngamma{Ngamma}_NR{NR}.npy')
psi_time = np.load(f'Pressure/psi_time_Neta{Neta}_NR{NR}.npy')

eta_component_train = np.transpose(eta_component_train, (1, 0, 2)) #(train_size, Neta, Ngamma)
eta_component_train = np.reshape(eta_component_train, (train_size, -1)) #(train_size, Neta*Ngamma)
psi_component = np.load(f'Pressure/psi_eta_Neta{Neta}_Ngamma{Ngamma}_NR{NR}.npy')
eta_component_test = np.transpose(eta_component_test, (1, 0, 2))
eta_component_test = np.reshape(eta_component_test, (test_size, -1))

eta_time_train = np.load(f'Pressure/eta_time_Neta{Neta}_NR{NR}.npy') #(nt, train_size, Neta)
eta_time_mean = np.mean(eta_time_train, axis = 1) #(nt, Neta)
...
class DNN:
    ...
    def do_DNN(self):
        ...
        for n in range(self.N_NN):
            ...
            for e in range(self.n_epoch):
                ...
                with torch.no_grad():
                    ...
                    #loss_test_val=DNN.weighted_mse_loss(y_pred_test, y_real, weights_eta).detach().cpu().numpy()
                    loss_test.append(loss_test_val)
                
                if e % 100 == 0:
                    print(f'Epoch: {e:03d}, Train Loss: {loss.item():.4f}, Test Loss:{loss_test_val:.4f}')
            print('Training Done')
            # Plot loss curves
            ...
            plt.legend(loc='best')
            plt.grid(True)
            plt.savefig(f'DNN_p_all_inputs/Loss_curve_Nxi{self.Nxi_dnn}_NetaNgamma{self.Neta_Ngamma_dnn}_hs{self.hidden_size}_NR{self.NR}_lambdareg{self.lambda_reg}.png')
        ...
        model_filename = f'DNN_p_all_inputs/model_NR{self.NR}_Netadnn{self.Neta_Ngamma_dnn}.pth'
...
# lambda_reg_list=[0]
Nxi_new_list = [9]
Neta_dnn_list= [Neta * Ngamma]
hidden_size_list=[36, 60]
lambda_reg_list=[0.015, 0.02]
    ...
    for hidden_size in hidden_size_list:
        for Nxi_new in Nxi_new_list:
            for Neta_dnn in Neta_dnn_list:
                ...
                eta_train = eta_component_train.reshape((train_size, -1))
                eta_test = eta_component_test.reshape((test_size, -1))
                print('eta_train.shape',eta_train.shape)
                print('eta_test.shape',eta_test.shape) 
                
                #Use DNN class
                file_eta_component_train = f'DNN_p_all_inputs/pred_eta_component_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_train.npy'	
                file_eta_component_test = f'DNN_p_all_inputs/pred_eta_component_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_test.npy'
                ...
                model.do_DNN()
                eta_train_pred = np.mean(np.load(file_eta_component_train), axis = 0)  #(train_size,Neta_dnn * Ngamma)
                eta_test_pred = np.mean(np.load(file_eta_component_test), axis = 0)
                
                eta_component_train_pred = np.reshape(np.load(file_eta_component_train), (train_size, Neta, Ngamma))  #it was (train_size, Neta_dnn)
                eta_component_test_pred = np.reshape(np.load(file_eta_component_test), (test_size, Neta, Ngamma))
                ...
                for i, test in enumerate(test_case):
                    ...
                    plt.savefig(f'DNN_p_all_inputs/eta_component_s_comparison_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_test{test}.png', dpi=300)
                    plt.close() 
                ...
                plt.legend()
                plt.title('Eta_component Comparison')
                plt.savefig(f'DNN_p_all_inputs/eta_component_s_comparison_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_train.png', dpi=300)
                plt.close()
                ...
                for j in range(Neta):
                    eta_rec_test[:, :, j] = (eta_time_mean[:, j] + (psi_component[j, :, :] @ eta_component_test_pred[:, j, :].T).T).T   
                
                for i in range(nt):
                    p_test_pred[:, i, :] = p_1_mean[i, :] + (psi_time[i, :, :Neta] @ eta_rec_test[i, :, :].T).T
                    
                # Calculate Errors - TESTING CCS1
                errors_linear_test = np.zeros((test_size, 4))
                for i, test in enumerate(test_case):
                    u_true = p_1_test[i]
                    ...
                    errors_linear_test[i, 2] = np.max(np.mean(np.abs(error)))
                    errors_linear_test[i, 3] = np.max(np.mean(np.abs(error_mean)))
                avg_errors_test = np.mean(errors_linear_test, axis=0)
                np.save(f'DNN_p_all_inputs/errors_s_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_test.npy', errors_linear_test)
                # Plotting TESTING Errors
                plt.figure(dpi=300)
                plt.scatter(test_case, errors_linear_test[:, 1], label='Average RMSE', color='blue', marker='o')
                plt.scatter(test_case, errors_linear_test[:, 0], label='DNN RMSE', color='red', marker='o')
                plt.scatter(test_case, errors_linear_test[:, 3], label='Average MAE', color='purple', marker='x')
                plt.scatter(test_case, errors_linear_test[:, 2], label='DNN MAE', color='crimson', marker='x')
                plt.xlabel('Test Case')
                plt.ylabel('Pressure Error (psi)')
                plt.legend()
                plt.savefig(f'DNN_p_all_inputs/errors_s_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_test.png')
                plt.close()
                del p_test_pred
                ...
                eta_rec_train = np.zeros((nt, train_size, Neta))
                for j in range(Neta):
                    eta_rec_train[:, :, j] = (eta_time_mean[:, j] + (psi_component[j, :, :] @ eta_component_train_pred[:, j, :].T).T).T   
                
                for i in range(nt):
                    p_train_pred[:, i, :] = p_1_mean[i, :] + (psi_time[i, :, :Neta] @ eta_rec_train[i, :, :].T).T
                    

                 # Calculate Errors - TRAINING CCS1
                errors_linear_test = np.zeros((train_size, 4))
                for i, test in enumerate(train_case):
                    ...
                    errors_linear_test[i, 3] = np.max(np.mean(np.abs(error_mean)))
                avg_errors_test = np.mean(errors_linear_test, axis=0)
                np.save(f'DNN_p_all_inputs/errors_s_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_train.npy', errors_linear_test)
                # Plotting TESTING Errors
                plt.figure(dpi=300)
                plt.scatter(train_case, errors_linear_test[:, 1], label='Average RMSE', color='blue', marker='o')
                plt.scatter(train_case, errors_linear_test[:, 0], label='DNN RMSE', color='red', marker='o')
                plt.scatter(train_case, errors_linear_test[:, 3], label='Average MAE', color='purple', marker='x')
                plt.scatter(train_case, errors_linear_test[:, 2], label='DNN MAE', color='crimson', marker='x')
                plt.xlabel('Train Case')
                plt.ylabel('Pressure Error (psi)')
                plt.legend()
                plt.savefig(f'DNN_p_all_inputs/errors_s_Nxi{Nxi_dnn}_Neta{Neta_dnn}_hs{hidden_size}_NR{NR}_lambdareg{lambda_reg}_train.png')
                plt.close()
